...
void insertMaxHeap(HeapType* h, element item){//시험!!!!!!!
	int i;
	i = ++(h->heap_size);
	...
	/**/h->heap[i] = item;//새로운 노드 삽입 heap[5]=8;
}
element deleteMaxHeap(HeapType* h){ 
	int parent, child;//인덱스
	element item, temp;//키값
	item = h->heap[1];//루트노드 반환값 
	temp = h->heap[h->heap_size];//말단노드를 저장 3키 저장 //이거 잘봐라!!!!
	h->heap_size--;
	parent = 1;
	child = 2;
	while(child <= h->heap_size){
		//현재 노드의 자식 노드 중 더 큰 자식 노드를 검색
		if (child < h->heap_size && h->heap[child] < h->heap[child+1]/*왼쪽노드가 더 작으면*/ )
			child++;
